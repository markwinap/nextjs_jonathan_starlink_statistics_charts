...
const fetchHtml = async (timestamp: string) => {
  const response = await fetch(`https://web.archive.org/web/${timestamp}/https://planet4589.org/space/con/star/stats.html`);
...
export async function GET(request: Request) {
  ...
  const timestamp = searchParams.get('timestamp') || '20240601'; // default to June 1, 2024
  console.log('searchParams', searchParams);
  console.log(`Fetching archive data for timestamp: ${timestamp}`);
  // fetch the html from the archive
  const html = await fetchHtml(timestamp);
